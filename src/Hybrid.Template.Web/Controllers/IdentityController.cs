// -----------------------------------------------------------------------
//  <copyright file="IdentityController.cs" company="Hybrid开源团队">
...
using Hybrid.Template.Identity;
    ...
    public class IdentityController : ApiController
    {
        ...
        public async Task<AjaxResult> Register(RegisterDto dto)
        {
            ...
            if (result.Succeeded)
            {
                ...
                string body =
                    ...
                    + $"{url}<br>"
                    + $"祝您使用愉快！";
                await SendMailAsync(user.Email, "cn.lxking 注册邮箱激活邮件", body);
        ...
        public async Task<AjaxResult> SendResetPasswordMail(SendMailDto dto)
        {
            ...
            string body = $"亲爱的用户 <strong>{user.NickName}</strong>[{user.UserName}]，您好！<br>"
                + $"欢迎使用cn.lxking账户密码重置功能，请 <a href=\"{url}\" target=\"_blank\"><strong>点击这里</strong></a><br>"
                + $"如果上面的链接无法点击，您可以复制以下地址，并粘贴到浏览器的地址栏中打开。<br>"
                + $"{url}<br>"
                + $"祝您使用愉快！";
            await sender.SendEmailAsync(user.Email, "cn.lxking 重置密码邮件", body);

// -----------------------------------------------------------------------
//  <copyright file="InfosService.Message.cs" company="Hybrid开源团队">
...
using Hybrid.Template.Infos.Dtos;
using Hybrid.Template.Infos.Entities;
using Hybrid.Template.Infos.Events;
...
namespace Hybrid.Template.Infos
{
        ...
        public override async Task<OperationResult> CreateMessages(params MessageInputDto[] dtos)
        {
            ...
            OperationResult result = await MessageRepository.InsertAsync(dtos,
                dto =>
                {
                    ...
                    {
                        throw new HybridException("私有消息的接收用户不能为空");
                    }
                ...
                async (dto, entity) =>
                {
                    ...
                    if (sender == null)
                    {
                        throw new HybridException($"编号为“{dto.SenderId}”的发件人不存在");
                    }
